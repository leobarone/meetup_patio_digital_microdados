...
No exemplo anterior passamos rapidamente por uma quantidade grande de funções e características da linguagem R e produzimos um exemplo simples a partir de dados deeducação do Portal de Dados Abertos da Prefeitura de São Paulo.
...
```{r}
matriculas <- group_by(matriculas, turma) %>%  summarise(contagem = sum(deficiencia), total = n())
```

Imediatamente a seguir, faremos um "sumário" da variável "deficiencia". Como queremos contar o número de alun@s com deficiência em cada turma, o "sumário" deverá conter uma variável "contagem". Também será interessante contar quantos studanteso todo nas turmas, informação que guardaremosna variável "total":
...
Novamente, não há tempo para explorar a gramática de gráficos do pacote _ggplot2_ na oficina, mas deixo indicado no repositório onde aprender um pouco mais.